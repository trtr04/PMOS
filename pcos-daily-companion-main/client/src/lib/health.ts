...
export async function getHistory(startDate: string, endDate: string) {
  const user = await workspace();
  const [{ data: templates, error: templateError }, { data: checkins, error: checkinError }, { data: cycles, error: cycleError }, { data: symptoms, error: symptomError }] = await Promise.all([
    supabase.from("habit_templates").select("id,score_value").eq("user_id", user.id).eq("is_active", true),
    supabase.from("daily_checkins").select("record_date,template_id,completed").eq("user_id", user.id).gte("record_date", startDate).lte("record_date", endDate),
    supabase.from("menstrual_cycles").select("start_date,end_date").eq("user_id", user.id).lte("start_date", endDate),
    supabase.from("cycle_symptoms").select("record_date,pain,breast_swelling,acne").eq("user_id", user.id).gte("record_date", startDate).lte("record_date", endDate),
  ]);
  if (templateError || checkinError || cycleError || symptomError) throw templateError || checkinError || cycleError || symptomError;
  const scoreByTemplate = new Map((templates ?? []).map((row: any) => [row.id, row.score_value])); const entries = new Map<string, any[]>();
  for (const item of checkins ?? []) (entries.get(item.record_date) ?? entries.set(item.record_date, []).get(item.record_date)!).push(item);
  const symptomByDate = new Map((symptoms ?? []).map((item: any) => [item.record_date, { pain:item.pain, breastSwelling:item.breast_swelling, acne:item.acne }]));
  const dates: string[] = []; for (let day = new Date(`${startDate}T12:00:00`), end = new Date(`${endDate}T12:00:00`); day <= end; day.setDate(day.getDate()+1)) dates.push(day.toISOString().slice(0,10));
  return dates.map(date => { const dayEntries = entries.get(date) ?? []; const complete = dayEntries.filter(row => row.completed); const total = templates?.length ?? 0; const score = Math.min(100, complete.reduce((sum, row) => sum + Number(scoreByTemplate.get(row.template_id) ?? 0), 0)); const isPeriodDay = (cycles ?? []).some((cycle: any) => cycle.start_date <= date && (!cycle.end_date || cycle.end_date >= date)); return { date, metric:{healthScore:score,completionRate:total ? Math.round(complete.length/total*100):0,completedCount:complete.length,totalCount:total},isPeriodDay,symptoms:symptomByDate.get(date) ?? null }; });
}
