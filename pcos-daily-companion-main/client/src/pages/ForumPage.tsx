...
type Post = { id: string; author_id: string; author_name: string; title: string; content: string; tag: Tag; created_at: string; comments: { id: string }[]; likes: { user_id: string }[] };
...
export default function ForumPage() {
  ...
  const load = useCallback(async () => {
    try {
      ...
      const { data, error } = await supabase.from("posts").select("id,author_id,author_name,title,content,tag,created_at,comments(id),likes(user_id)").order("created_at", { ascending: false }).limit(50);
